...
function setupEventListeners() {
    ...
    ['filterQuoteBook', 'filterQuoteMovie', 'filterQuoteAssorted'].forEach(id => {
...
async function fetchSuggestions(search, type, container, input) {
    try {
        ...
        const response = await fetch(`${API_URL}/${endpoint}?search=${encodeURIComponent(search)}`);
...
async function loadQuotes() {
    try {
        ...
        // Add type filter
        const selectedTypes = [];
        if (document.getElementById('filterQuoteBook')?.checked) selectedTypes.push('BOOK');
        if (document.getElementById('filterQuoteMovie')?.checked) selectedTypes.push('MOVIE');
        if (document.getElementById('filterQuoteAssorted')?.checked) selectedTypes.push('ASSORTED');
...
async function loadTotalCount() {
    try {
        ...
        const filteredResponse = await fetch(`${API_URL}/quotes/count?${params.toString()}`);
...
async function handleSubmit(e) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            alert('Failed to save quote: ' + (errorData.error || 'Please try again.'));
        }
    } catch (error) {
...
function displayQuotes(quotes) {
    ...
    if (quotes.length === 0) {
        quotesList.innerHTML = '<div class="no-quotes">No quotes found. Add your first quote!</div>';
        return;
    }
    ...
    document.querySelectorAll('.source-link').forEach(link => {
        link.addEventListener('click', (e) => {
            e.preventDefault();
            openSourceModal(link.dataset.id, link.dataset.name, link.dataset.type || 'BOOK');
        });
    });
}

function createQuoteCard(quote) {
    const tags = quote.tags ? quote.tags.split(',').map(tag => 
    ...
    const sourceIcon = sourceType === 'MOVIE' ? '🎬' : sourceType === 'ASSORTED' ? '📝' : '📖';
    
    // Check if quote is long (more than 10 lines)
    const lines = quote.quote.split('\n');
    const isLongQuote = lines.length > 10;
    const shortQuote = isLongQuote ? lines.slice(0, 10).join('\n') + '\n...' : quote.quote;
...
document.getElementById('deleteAuthorBtn').addEventListener('click', handleDeleteAuthor);
document.getElementById('deleteSourceBtn').addEventListener('click', handleDeleteSource);
...
function previewBulkQuotes() {
    ...
    if (quotes.length === 0) {
        alert('No quotes found. Make sure to separate quotes with --- on its own line.');
        return;
    }
...
async function handleBulkSubmit(e) {
    ...
    if (!confirm(`Add ${quotes.length} quotes by ${author}${source ? ' from ' + source : ''}?`)) {
        return;
    }
...
function debounceAutocomplete(value, type) {
    ...
    autocompleteTimeout = setTimeout(() => {
        if (value.length < 2) {
            ...
            else if (type === 'bulkAuthor') bulkAuthorSuggestions.classList.remove('show');
            else if (type === 'bulkSource') bulkSourceSuggestions.classList.remove('show');
        ...
        if (type === 'author') {
            ...
            fetchSuggestions(value, 'authors', bulkAuthorSuggestions, bulkAuthorInput);
        } else if (type === 'bulkSource') {
            fetchSuggestions(value, 'sources', bulkSourceSuggestions, bulkSourceInput);
        } else if (type === 'tags') {
    ...
    container.querySelectorAll('.autocomplete-item').forEach(item => {
        item.addEventListener('click', () => {
            ...
            if (lastCommaIndex >= 0) {
                ...
                newValue = fullValue.substring(0, lastCommaIndex + 1) + ' ' + item.dataset.value;
            } else {
...
function updatePaginationControls() {
    ...
    const startItem = filteredQuotes === 0 ? 0 : (currentPage - 1) * quotesPerPage + 1;
...
async function loadAuthors() {
    try {
        ...
        const searchTerm = document.getElementById('searchAuthorName')?.value.toLowerCase().trim();
        if (searchTerm) {
        ...
        if (sortBy === 'name') {
            ...
            authors.sort((a, b) => (parseInt(b.quote_count) || 0) - (parseInt(a.quote_count) || 0));
        }
        
        displayAuthors(authors);
    } catch (error) {
        console.error('Error loading authors:', error);
        document.getElementById('authorsList').innerHTML = '<div class="no-items">Failed to load authors.</div>';
    }
}
...
async function loadSources() {
    try {
        ...
        // Filter by search term
        const searchTerm = document.getElementById('searchSourceName')?.value.toLowerCase().trim();
        if (searchTerm) {
            sources = sources.filter(source => 
        ...
        if (sortBy === 'name') {
            ...
            sources.sort((a, b) => (parseInt(b.quote_count) || 0) - (parseInt(a.quote_count) || 0));
        }
        
        displaySources(sources);
    } catch (error) {
        console.error('Error loading sources:', error);
        document.getElementById('sourcesList').innerHTML = '<div class="no-items">Failed to load sources.</div>';
    }
}
...
function displaySources(sources) {
    ...
    sourcesList.innerHTML = sources.map(source => {
        const typeIcon = source.type === 'MOVIE' ? '🎬' : source.type === 'ASSORTED' ? '📝' : '📖';
        return `
...
async function loadTags() {
    try {
        ...
        document.getElementById('tagsList').innerHTML = '<div class="no-items">Failed to load tags.</div>';
    }
}
