...
|--------|---------|------------|--------------|
| `setup-sqlite-install.js` | First-run local SQLite setup: writes `.env`, `config/local.json`, creates vault/data folders, copies the `inst/sqlite-standard/` profile or seed-vault settings/palettes, and runs migrations. `--db` accepts either a directory for `archive.sqlite` or an explicit DB file. | `npm run setup-sqlite -- --vault ~/Documents/NoteArchiveVault --db ~/NoteArchiveData` (`--seed-vault`, `--include-attachments`, `--force`, `--dry-run`, `--no-migrate`) | Node 24+ for SQLite runtime |
| `make-sqlite-package.js` | Build a clean source tarball for a SQLite single-user install, excluding secrets, DB files, `node_modules`, and Git metadata. Includes `inst/sqlite-standard/`; can additionally bundle seed-vault settings/palettes and optionally attachments. | `npm run package-sqlite -- --seed-vault /path/to/Vault --include-attachments` | `tar` |
...
| `full-backup.js` | Create one checksummed `.tar.gz` containing an online PostgreSQL/SQLite database snapshot, the complete attachment tree, and vault settings/palettes. | `npm run backup` (`--output`, `--backend`, `--attachments`, `--sqlite-path`, `--dry-run`) | `tar`; `pg_dump` for PostgreSQL |
| `full-restore.js` | Verify a full backup, then optionally restore its database and complete attachment tree. Inspection is the default; `--apply` is required. Existing SQLite/attachment data is retained with a `.before-restore-*` name. | `npm run restore -- backups/note-archive-full-….tar.gz`; then repeat with `--apply` after stopping the app | `tar`; `psql` for PostgreSQL |
