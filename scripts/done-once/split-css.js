...
/* eslint-disable */
/**
 * Archival one-shot: split a monolithic `public/style.css` into base + mobile + medium.
 * The repo already ships split files — do not re-run unless you are deliberately
 * re-merging and re-splitting CSS.
 *
 * Outputs:
 *   - public/style.css         (base / desktop, no @media blocks)
 *   - public/style.mobile.css  (all max-width @media queries)
 *   - public/style.medium.css  (768px–1100px band)
 *
 * From repository root:
 *   node scripts/done-once/split-css.js
 *
 * Does not emit `style.small.css` (hand-maintained); keep it linked in
 * `index.html` after `style.mobile.css`.
 */

const fs = require('fs');
const path = require('path');

/** Repo root (this file lives in scripts/done-once/) */
...
const banner = (label) => `/* ============================================================
   ${label}
   Generated by scripts/done-once/split-css.js — see ARCHITECTURE.md for the
   responsive-CSS file layout.  Order in index.html matters:
       style.css  →  style.mobile.css  →  style.small.css  →  style.medium.css
   (style.small.css is hand-maintained for max-width: 480px; not emitted here.)
   ============================================================ */\n\n`;
