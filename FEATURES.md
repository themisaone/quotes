...
**List + pane view:** Alternative to the card grid — compact list on the left, full note in the right pane (reuses the edit modal). **Training** notes always use this layout (no Cards option); the left column can switch between a **list** and a **month calendar** (sub-type **icons** in each day cell). **All Notes** and **every other note type** (except Training) can switch between **Cards** and **List + Pane** via the **DISPLAY_MODE** dropdown in the page header (before **Add New**); list rows show title (or **No title**), two lines of note text, and an **80×80px** thumbnail when an attachment exists (no placeholder when absent; row height stays the same); every row is the same height. Up to **20** notes per page in list-pane mode for non-training types (training uses 12). The right pane matches the left list height and scrolls internally; it shows the normal full card. Per-type preference is stored in `localStorage` (`viewMode_all` for **All Notes**, `viewMode_<type>` per type). Implemented in `listPaneView.js` + `trainingCalendar.js`.
