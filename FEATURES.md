...
Left sidebar lists note types grouped by mode. Each type is a link that sets the URL hash and filters the card list. The menu can be collapsed with the ◀ toggle (desktop/medium). On phone-width screens (**≤767px**), when the active mode includes **more than one** note type, a **note-type dropdown** appears in the quotes header (beside Cards / Add New) — hidden in single-type instances (`npm run tegneserie`, etc.).
